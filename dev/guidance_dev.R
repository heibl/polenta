...
guidance <- function(sequences,
  ...
  gt.program ="njR",
  model = "JC69",
  semphy.exec = "/Applications/semphy-2.0b2-bin-mac-intel/semphy"){
    ...
    if (parallel){
      ...
      nj.guide.trees <- foreach(i = 1:bootstrap,
        ...
        .packages = "phangorn", .export = 'msaBP_nj_tree') %dopar% {
          msaBP_nj_tree(base.msa, outgroup = "auto", model = model)
        }
      stopCluster(cl)
    }
    if (!parallel){
  ...
  ## Here add new Rcpp function performing the scores computation
  #---------------------------------------------------------------
  # [first using function *add_msa*]

  library(Rcpp)
  sourceCpp("src/res_pair_score.cpp")

  # experimental:
  bm <- cmatrix(base.msa)
  am <- cmatrix(read.fas(list.files(paste(tempdir(), "alt", sep="/"), full.names = TRUE)[2]))
  system.time(
  res <- add_msa(bm, am)
)
